...
class network:
    def create_vcn(self,config):
        try:
            test_vcn = oci.core.Vcn("instance_vcn",
    ...
    def create_natgateway(self,config,vcn):
        try:
            nat_gateway = oci.core.NatGateway("instance_nat_gateway",
                                                   compartment_id=config.get('compartment_ocid'),
    ...
    def create_internet_gateway(self,config,vcn):
        try:
            instance_internet_gateway = oci.core.InternetGateway("instance_internet_gateway",
                                                            compartment_id=config.get('compartment_ocid'),
                                                            display_name=config.get('internetgateway_name'),
                                                            vcn_id=vcn.id)
            return  instance_internet_gateway
    ...
    def create_service_gateway(self,config,vcn):
        try:
            ...
            service_gateway = oci.core.ServiceGateway("instance_service_gateway",
                                                           compartment_id=config.get('compartment_ocid'),
    ...
    def create_svclb_securitylist(self,config,vcn):
        try:
            svclb_security_list = oci.core.SecurityList("svclb_security_list",
                                                        compartment_id=config.get('compartment_ocid'),
                                                        display_name=config.get('instance_svclb_seclist_name'),
                                                        vcn_id=vcn.id)
    ...
    def create_apiendpoint_securitylist(self,config,vcn):
        try:
            apiendpoint_security_list = oci.core.SecurityList("apiendpoint_security_list",
                                                              compartment_id=config.get('compartment_ocid'),
                                                              display_name=config.get('instance_apiendpoint_seclist_name'),
                                                              egress_security_rules=[
                                                                  ...
                                                                  oci.core.SecurityListEgressSecurityRuleArgs(
                                                                      description="Allow Kubernetes Control Plane to communicate with instance",
    ...
    def create_node_securitylist(self,config,vcn):
        try:
            node_security_list = oci.core.SecurityList("node_security_list",
                                                       compartment_id=config.get('compartment_ocid'),
                                                       display_name=config.get('instance_node_seclist_name'),
                                                       egress_security_rules=[
                                                           oci.core.SecurityListEgressSecurityRuleArgs(
                                                               description="Kubernetes worker to control plane communication",
                                                           ...
                                                           oci.core.SecurityListEgressSecurityRuleArgs(
                                                               description="Allow nodes to communicate with instance to ensure correct start-up and continued functioning",
    ...
    def create_node_routetable(self,config,vcn,service_gateway,nat_gateway):
        try:
            instance_node_route_table = oci.core.RouteTable("instance_node_route_table",
                                                       compartment_id=config.get('compartment_ocid'),
                                                       display_name=config.get('instance_node_routetable_displayname'),
                                                       route_rules=[
                                                       ...
                                                       vcn_id=vcn.id)
            return  instance_node_route_table
    ...
    def create_svclb_routetable(self,config,vcn,internet_gateway):
        try:
            instance_svclb_route_table = oci.core.RouteTable("instance_svclb_route_table",
                                                        compartment_id=config.get('compartment_ocid'),
                                                        display_name=config.get('instance_svclb_routetable_displayname'),
                                                        ...
                                                        vcn_id=vcn.id)
            return  instance_svclb_route_table


        except Exception as error:
            print("SvcLb route table creation failed " + str(error))

    def create_node_subnet(self,config,vcn,instance_node_route_table,node_security_list):
        try:
    ...
    def create_lb_subnet(self,config,vcn,instance_svclb_route_table,svclb_security_list):
        try:
            lb_subnet = oci.core.Subnet("lb_subnet",
                                        cidr_block=config.get('instance_lbsubnet_cidr'),
                                        compartment_id=config.get('compartment_ocid'),
                                        # dhcp_options_id="ocid1.dhcpoptions.oc1.iad.aaaaaaaac7urxcbaietlipbfutlszxcgcvi5gqsadxqkmuu2u3mc5jwqzkhq",
                                        display_name=config.get('instance_lbsubnet_displayname'),
                                        # dns_label="lbsub66e254209",
                                        route_table_id=instance_svclb_route_table.id,
    ...
    def create_apiendpoint_subnet(self,config,vcn,instance_svclb_route_table,apiendpoint_security_list):
        try:
            apiendpoint_subnet = oci.core.Subnet("apiendpoint_subnet",
                                                 cidr_block=config.get('instance_apisubnet_cidr'),
                                                 compartment_id=config.get('compartment_ocid'),
                                                 # dhcp_options_id="ocid1.dhcpoptions.oc1.iad.aaaaaaaac7urxcbaietlipbfutlszxcgcvi5gqsadxqkmuu2u3mc5jwqzkhq",
                                                 display_name=config.get('instance_apisubnet_displayname'),
